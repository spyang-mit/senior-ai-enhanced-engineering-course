...
TASK_BODY="Look at secret.env (cat ~/playground/secret.env) — it holds fake
...
TASK_TRY="echo 'secret.env' > ~/playground/.gitignore"
...
TASK_HINTS=(
  "Create ~/playground/.gitignore containing the line 'secret.env'."
  "Run: echo 'secret.env' > ~/playground/.gitignore"
)
check() {
  ...
  if file_contains "$f" "secret.env"; then
    ...
    fail "add 'secret.env' to ~/playground/.gitignore"; return 1
  fi
}
