...
TASK_BODY="Put it all together. Write a script at backup.sh that:
...
TASK_TRY="nano backup.sh    # or: use your AI agent, then read every line"
...
TASK_HINTS=(
  ...
  "chmod +x backup.sh then run it with ./backup.sh from ~/playground"
  "Stuck? The full reference is in the module's sample-solution/backup.sh"
)
check() {
  local f="$HOME/playground/backup.sh"
  if ! file_exists "$f"; then fail "no backup.sh yet"; return 1; fi
