...
Create a COMPRESSED archive of your notes directory:
  cd ~/playground
  tar czf notes.tar.gz notes"
TASK_TRY="cd ~/playground && tar czf notes.tar.gz notes"
...
TASK_HINTS=(
  ...
  "Run: cd ~/playground && tar czf notes.tar.gz notes"
)
check() {
  ...
  if gzip_tar_valid "$f" && gzip_tar_contains "$f" "notes/"; then
    ...
    fail "expected a valid gzip archive at ~/playground/notes.tar.gz containing notes/"; return 1
  fi
}
