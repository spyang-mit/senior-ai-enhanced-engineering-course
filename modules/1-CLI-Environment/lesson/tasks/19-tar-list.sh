...
TASK_TRY="tar tzf ~/playground/notes.tar.gz > ~/playground/contents.txt"
...
TASK_HINTS=(
  ...
  "Run: tar tzf ~/playground/notes.tar.gz > ~/playground/contents.txt"
)
check() {
  ...
  if file_contains "$f" "notes/"; then
    ...
    fail "save the output of 'tar tzf notes.tar.gz' to ~/playground/contents.txt"; return 1
  fi
}
