...
scatter files into your current one:
  mkdir -p ~/playground/restored
  tar xzf ~/playground/archive/oldlogs.tar.gz -C ~/playground/restored

  x = extract   z = gunzip   f = file    -C = change to this dir first"
TASK_TRY="mkdir -p ~/playground/restored && tar xzf ~/playground/archive/oldlogs.tar.gz -C ~/playground/restored"
...
check() {
  ...
  if [ -d "$d" ] && [ -n "$(find "$d" -type f 2>/dev/null | head -1)" ]; then
    pass "extracted the old logs into ~/playground/restored/ — files are there."
  else
    fail "expected extracted files under ~/playground/restored/"; return 1
  fi
}
