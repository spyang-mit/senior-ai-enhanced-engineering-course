...
Save a detailed listing so we can dissect it. Capture it to a file:"
TASK_TRY="ls -al ~/playground > ~/playground/listing.txt"
TASK_WHY="That '>' sends the output into a file instead of the screen (more
on that next). The README breaks down every column of an 'ls -al' line —
read it; those columns are how you diagnose permission bugs."
TASK_HINTS=(
  "Combine the flags: ls -al, and redirect with >."
  "Run: ls -al ~/playground > ~/playground/listing.txt"
)
check() {
  ...
  if file_nonempty "$f" && file_contains "$f" "^total "; then
    ...
    fail "no 'ls -al' output found in ~/playground/listing.txt yet"
    return 1
  fi
}
