...
TASK_BODY="shared/report.txt is currently rw-r--r--: owner can edit, everyone
can read. You want your teammates (group) to be able to EDIT it, and you want
to stop the general public (others) from reading it.

  chmod g+w file    add write for group
  chmod o-r file    remove read from others
  chmod o+r file    (the opposite — add read for others)
(You can apply several changes in one command, separated by commas.)

Your task: make shared/report.txt writable by the group and NOT readable by
others."
TASK_TRY=""
...
TASK_HINTS=(
  "You need group write ON and others read OFF — i.e. end state rw-rw----."
  "Run: chmod g+w,o-r shared/report.txt"
)
check() {
