...
missing-resource from empty-result. Fix the codes in:
  ~/workspace/handlers/07-codes.py
...
TASK_HINTS=(
  "In ~/workspace/handlers/07-codes.py there are two TODOs: the POST branch and the GET branch."
  "A successful create returns (201, order); a GET with no matching order returns (404, {\"error\": ...})."
)
TASK_GOAL="Return 201 on create and 404 on a missing order — edit ~/workspace/handlers/07-codes.py"
setup() { seed_handler "07-codes.py"; }
check() {
  if run_harness codes "$HANDLERS/07-codes.py"; then
    pass "create -> 201, missing -> 404. Your codes now tell the truth about what happened."
  else
    fail "fix the status codes in ~/workspace/handlers/07-codes.py (see the failing checks above), then run lesson check"
    return 1
  fi
}
