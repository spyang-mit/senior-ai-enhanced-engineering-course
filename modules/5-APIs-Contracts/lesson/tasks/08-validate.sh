...
Now harden it. Your create handler lives in the mounted workspace:
  ~/workspace/handlers/validate.py
...
TASK_HINTS=(
  "Open ~/workspace/handlers/validate.py — the TODO marks exactly where to add checks, before the order is built."
...
TASK_GOAL="Make POST /orders reject bad input with 400 — edit ~/workspace/handlers/validate.py"
setup() { seed_handler "validate.py"; }
check() {
  if run_harness validate "$HANDLERS/validate.py"; then
    ...
    fail "edit ~/workspace/handlers/validate.py so bad input returns 400 (see the failing checks above), then run lesson check"
    return 1
  fi
}
