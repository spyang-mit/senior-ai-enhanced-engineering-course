...
TASK_BODY="Close the hole you just found. In:
  ~/workspace/handlers/12-price.py
...
TASK_TRY="less ~/workspace/handlers/12-price.py"
...
TASK_GOAL="Compute totalCents on the server, ignoring the client — edit ~/workspace/handlers/12-price.py"
setup() { seed_handler "12-price.py"; }
check() {
  if run_harness price "$HANDLERS/12-price.py"; then
    pass "the forged total is ignored; the server prices the order from its own catalog. The trust boundary holds."
  else
    fail "make ~/workspace/handlers/12-price.py compute the total server-side (see failing check above), then run lesson check"
    return 1
  fi
}
