...
Fix it in:
  ~/workspace/handlers/pagination.py
...
TASK_TRY="less ~/workspace/handlers/pagination.py"
...
TASK_GOAL="Return only the caller's orders as {total, items}, honoring limit/offset — edit ~/workspace/handlers/pagination.py"
setup() { seed_handler "pagination.py"; }
check() {
  if run_harness pagination "$HANDLERS/pagination.py"; then
    pass "the list is now scoped to the caller and paginated, with an honest total. It'll survive real data volumes."
  else
    fail "make ~/workspace/handlers/pagination.py filter + paginate + return {total, items} (see failing checks above), then run lesson check"
    return 1
  fi
}
