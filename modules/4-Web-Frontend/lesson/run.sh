...
load_task() {
  ...
  # A task is either a "check" task (defines check()) or a "quiz" task (sets
  # TASK_QUIZ + TASK_QUIZ_OPTIONS + TASK_QUIZ_ANSWER, and lesson check asks it).
...
# Announce success and advance to the next task (shared by check and quiz).
_advance() {
  local n=$1
  set_cur $((n + 1))
  echo
...
show_task() {
  ...
  # above. It states WHAT to produce, not the exact command (that's in the hint).
...
# Present the task's multiple-choice question, read an answer, validate it.
do_quiz() {
...
finished() {
  echo
  rule
  printf '%s\n' "${C_GREEN}${C_BOLD}🎉 You finished Module 4's guided lesson.${C_RESET}"
  rule
  cat <<EOF

You can read and write JSON, drive an HTTP API by hand (curl + jq), read an
OpenAPI spec written in YAML as a contract, tell a request apart from a response
and a client from a server, and use the browser's Network tab as ground truth —
the literacy you need to judge and verify AI-built frontends, not hand-build them.

Next: the project in README.md — vibe-code a contacts frontend against the spec,
then let the backend's access log prove it works. Also try the quizzes.
...
A contacts API is running at ${C_BOLD}http://localhost:8080${C_RESET} — curl it here, and open it in
your own browser to use DevTools. It speaks ${C_BOLD}JSON${C_RESET}; its contract is in
${C_BOLD}contacts-api.yaml${C_RESET}. Your job here is to CONSUME and VERIFY the web, not build it by hand.
